...
- It isn't a plain top-level function or `staticmethod` - a classmethod, instance method, nested function, or lambda is still skipped (see the "method support" known limitation below for why).
...
**Known limitation (method support):** a `staticmethod` is generatable the same way a plain top-level function is - both as the function under test (rendered as `ClassName.method_name(...)`, importing just the class) and as a mocked child call of another function under test. A classmethod or a plain instance method isn't generatable yet, for two different reasons. An instance method needs an instance to call it on, but the tracer only ever records `self` as an opaque `repr()` (an arbitrary instance isn't JSON-safe) - a natural next step would have the tracer additionally snapshot `self`'s own JSON-safe attribute state so the generator could rebuild an equivalent instance (via `ClassName.__new__(ClassName)` plus attribute assignment, never re-running `__init__`), rather than trying to identity-match a constructor call. A classmethod is a harder problem: its qualname always names the class it's *defined* on (e.g. `"Base.named"`), never the class actually used to call it - a call through a subclass still traces as `"Base.named"` with `cls` bound to the subclass, and the trace can't distinguish the two (`cls`'s own `repr()` fallback carries no class name at all, just a bare `"<type object at 0x...>"`). Rendering the call through the defining class could silently assert one class's behavior for what was actually traced through another - skipped entirely for now rather than risking a wrong or failing test, the same "skip over risk a wrong test" preference behind the list/tuple limitation above. Solving this properly needs the tracer to record the actual receiver's qualified name (not just an opaque repr), the same kind of tracer-level enhancement instance-method support would need anyway.

**Known limitation (mock patch target):** a mocked child is patched under the *calling* function's own name for it (e.g. `helper.round_result`), not the module it's actually defined in - the standard "patch where it's looked up" idiom. This assumes the caller reached the child via a plain `from its_module import name`. If the caller instead used `import its_module; its_module.name(...)`, the calling module has no attribute of that name, so `mock.patch()` raises `AttributeError` immediately - the generated test fails visibly rather than silently under-isolating. The one way this goes wrong silently: if the calling module happens to define or import some unrelated attribute under the same name as the child's qualname, while reaching the real child a different way, the mock patches the wrong thing without any error. This is accepted as a rare edge case; patching the child's own defining module instead was considered and rejected because it would require reimporting the calling module after the patch is applied, breaking the "each shared module is imported once per session" design the generated `conftest.py` relies on - and its own failure mode is silent (a stale, unpatched reference just keeps working) rather than loud. This already extends to a staticmethod child with no special-casing needed: patching `"helper.Widget.static_helper"` resolves through `Widget` to the method attribute directly, the same "patch where it's looked up" idiom applied one level deeper.
