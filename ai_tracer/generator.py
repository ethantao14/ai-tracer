...
def _render_conftest(target_dir):
    ...
    lines = [
        ...
        "# Never delete this conftest's own module or its ancestor packages:",
        "# when the output dir is a package under the target dir (e.g. a real",
        "# tests/ with __init__.py, possibly nested in a target package), those",
        "# names are target-local too, and deleting one mid-import aborts",
        "# collection with a KeyError. Only that exact ancestor chain is spared,",
        "# so a sibling target submodule (pkg.other) is still evicted.",
